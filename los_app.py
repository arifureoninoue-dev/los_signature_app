...
def generate_pdf():
    ...
    try:
        ...
        # 失敗しても、ユーザーにはPDFを返すので、こっそり記録するだけです。
        print(f"Error uploading completed PDF to Blob: {e}")

    # 「os」道具を使って、サーバーに一時保存した署名画像を「削除」します。（お掃除）
    os.remove(temp_signature_path)

    # 最後に、ユーザーに「完成したPDFデータ」を送り返します。
    return Response(
        pdf_output,  # これがPDFデータ本体です
        mimetype="application/pdf",  # 「これはPDFファイルですよ」と教えます
        # 「これは "confirmation_signed.pdf" という名前で「ダウンロード」させてください」とお願いします
